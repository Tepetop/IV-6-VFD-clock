 ...
 */
static DS3231_Status ds3231_read_regs(DS3231_t *hrtc, uint8_t reg, uint8_t *buf, uint8_t len)
{
    if (HAL_I2C_Master_Transmit(hrtc->hi2c, hrtc->i2c_addr, &reg, 1, DS3231_I2C_TIMEOUT) != HAL_OK) {
        return DS3231_ERR_I2C;
    }
    if (HAL_I2C_Master_Receive(hrtc->hi2c, hrtc->i2c_addr, buf, len, DS3231_I2C_TIMEOUT) != HAL_OK) {
        return DS3231_ERR_I2C;
    }
    return DS3231_OK;
}
 ...
 */
static uint8_t ds3231_encode_hours(uint8_t hours, DS3231_HourFormat fmt, DS3231_AmPm ampm)
{
    uint8_t reg = 0;
 ...
 */
static void ds3231_decode_hours(uint8_t reg, uint8_t *hours, DS3231_HourFormat *fmt, DS3231_AmPm *ampm)
{
    if (reg & (1 << 6)) {
        /* Tryb 12h */
